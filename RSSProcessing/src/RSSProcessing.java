...
public final class RSSProcessing {
    ...
    public static void main(String[] args) {
        ...
        String titleText = "Text Not Found";

        if (channel.child(getChildElement(channel, "title"))
                .numberOfChildren() > 0) {
            titleText = channel.child(getChildElement(channel, "title"))
                    .child(0).label();
        }
        out.println(titleText);

        String descText = "Text Not Found";

        if (channel.child(getChildElement(channel, "description"))
                .numberOfChildren() > 0) {
            descText = channel.child(getChildElement(channel, "title")).child(0)
                    .label();
        }
        out.println(descText);
